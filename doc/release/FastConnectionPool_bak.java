 ...
 * @version 1.0
 */
public final class FastConnectionPool implements ConnectionPool, ConnectionPoolJMXBean {
    ...
    private final AddConnectionTask addConnectionTask = new AddConnectionTask();
    private final LinkedBlockingQueue<Runnable> createConnTaskQueue = new LinkedBlockingQueue<Runnable>();
    private final ThreadPoolExecutor createConnExecutor = new ThreadPoolExecutor(1, 1, 10, SECONDS, createConnTaskQueue);
    ...
    public void close() throws SQLException {
        ...
        while (true) {
            if (poolState.compareAndSet(POOL_NORMAL, POOL_CLOSED)) {
                ...
                unregisterJMX();
                this.createConnExecutor.shutdownNow();
    ...
    private final void tryToCreateNewConnByAsyn() {
        if (connArray.length + createConnTaskQueue.size() < poolMaxSize)
            createConnExecutor.submit(addConnectionTask);
    }
    ...
    //create new connection then transfer to waiter
    final class AddConnectionTask implements Runnable {
        public void run() {
            try {
                PooledConnection pConn = createPooledConn(CONNECTION_USING);
                if (pConn != null) recycle(pConn);
            } catch (SQLException e) {
                transferException(e);
            }
        }
