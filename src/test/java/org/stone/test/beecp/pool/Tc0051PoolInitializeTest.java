 ...
 * @author Chris Liao
 */
public class Tc0051PoolInitializeTest {
    ...
    public void testNullConfig() throws Exception {
        try {
            ...
            Assertions.assertTrue(message != null && message.contains("Pool initialization configuration can't be null"));
        }
    }
    ...
    public void testInitializeFailOnReadyPool() throws Exception {
        ...
        if (thread1.getFailureException() != null)
            Assertions.assertEquals("Pool has already initialized or in initializing", thread1.getFailureException().getMessage());
        if (thread2.getFailureException() != null)
            Assertions.assertEquals("Pool has already initialized or in initializing", thread2.getFailureException().getMessage());
    }
    ...
    public void testPoolWorkMode() throws Exception {
        ...
        FastConnectionPool pool = new FastConnectionPool();
        pool.init(config);
        Assertions.assertEquals("fair", TestUtil.getFieldValue(pool, "poolMode"));
        pool.close();
        ...
        pool2.init(config2);
        Assertions.assertEquals("compete", TestUtil.getFieldValue(pool2, "poolMode"));
        pool2.close();
    }
    ...
    public void testMockGetNullFromDriver() throws Exception {
        ...
        try {
            ...
            Assertions.assertTrue(e.getMessage().contains("A unknown error occurred when created a connection"));
        }
        ...
        try {
            ...
            Assertions.assertTrue(e.getMessage().contains("A unknown error occurred when created an XA connection"));
        }
        ...
        pool3.init(config3);
        pool3.close();
        //String logs = logCollector.endCollectedStoneLog();
        //Assertions.assertTrue(logs.contains("Failed to create initial connections by async mode"));
    }
    ...
    public void testMockCatchSQLExceptionFromDriver() throws Exception {
        ...
        try {
            ...
            Assertions.assertTrue(e.getMessage().contains("Communications link failure"));
        }
    ...
    public void testMockCatchRuntimeExceptionFromDriver() throws Exception {
        ...
        try {
            ...
            Assertions.assertTrue(cause.getMessage().contains("A runtime exception"));
        }
    ...
    public void testMockCatchErrorFromDriver() throws Exception {
        ...
        try {
            ...
            Throwable cause = e.getCause();
            Assertions.assertInstanceOf(Error.class, cause);
            Assertions.assertTrue(cause.getMessage().contains("Unknown error"));
        }
    ...
    public void testMockPartlySuccess() {
        try {
            ...
            Assertions.assertInstanceOf(ConnectionCreateException.class, e);
            Assertions.assertEquals("the count of created connections has reached max", e.getMessage());
        }
