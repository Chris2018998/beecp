...
import org.stone.test.beecp.objects.MockNetBlockConnectionFactory;
import org.stone.test.beecp.objects.MockNetBlockXaConnectionFactory;
 ...
 * @author Chris Liao
 */
public class Tc0056NetStuckMockTest {
    ...
    public void testStuckInConnectionFactory() throws Exception {
        ...
        if (waitUtilWaiting(firstBorrower)) {
            ...
            firstBorrower.join();
            Assertions.assertTrue(firstBorrower.getFailureCause().getMessage().contains("A unknown error occurred when created a connection"));
        }
        ...
        if (waitUtilWaiting(secondBorrower)) {
            ...
            secondBorrower.join();
            Assertions.assertTrue(secondBorrower.getFailureCause().getMessage().contains("An interruption occurred when created a connection"));
        }
    }
    ...
    public void testStuckInXaConnectionFactory() throws Exception {
        ...
        if (waitUtilWaiting(firstBorrower)) {
            factory.unparkToExit(firstBorrower);
            firstBorrower.join();
            Assertions.assertTrue(firstBorrower.getFailureCause().getMessage().contains("A unknown error occurred when created an XA connection"));
        }
        ...
        if (waitUtilWaiting(secondBorrower)) {
            ...
            secondBorrower.join();
            Assertions.assertTrue(secondBorrower.getFailureCause().getMessage().contains("An interruption occurred when created an XA connection"));
        }
    }
}
