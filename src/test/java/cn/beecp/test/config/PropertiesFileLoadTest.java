...
public class PropertiesFileLoadTest extends TestCase {
    public void test() throws Exception {
        ...
        if(!"com.mysql.cj.jdbc.Driver".equals(testConfig.getDriverClassName()))throw new BeeDataSourceConfigException("driverClassName error");
        if(!"test1".equals(testConfig.getDefaultCatalog()))throw new BeeDataSourceConfigException("defaultCatalog error");
        if(!"test2".equals(testConfig.getDefaultSchema()))throw new BeeDataSourceConfigException("defaultSchema error");
        ...
        if(testConfig.getConnectionTestTimeout()!=3)throw new BeeDataSourceConfigException("connectionTestTimeout error");
        if(testConfig.getConnectionTestInterval()!=500)throw new BeeDataSourceConfigException("connectionTestInterval error");
        if(testConfig.getIdleCheckTimeInterval()!=30000)throw new BeeDataSourceConfigException("idleCheckTimeInterval error");
        if(!testConfig.isForceCloseUsingOnClear())throw new BeeDataSourceConfigException("forceCloseUsingOnClear error");
        if(testConfig.getDelayTimeForNextClear()!=3000)throw new BeeDataSourceConfigException("delayTimeForNextClear error");
        if(!"cn.beecp.pool.DriverConnectionFactory".equals(testConfig.getConnectionFactoryClassName()))throw new BeeDataSourceConfigException("connectionFactoryClassName error");
        if(!"cn.beecp.xa.Mysql8XaConnectionFactory".equals(testConfig.getXaConnectionFactory().getClass().getName()))throw new BeeDataSourceConfigException("xaConnectionFactory error");
        if(!"cn.beecp.xa.Mysql8XaConnectionFactory".equals(testConfig.getXaConnectionFactoryClassName()))throw new BeeDataSourceConfigException("xaConnectionFactoryClassName error");
        if(!"cn.beecp.pool.RawConnectionPool".equals(testConfig.getPoolImplementClassName()))throw new BeeDataSourceConfigException("poolImplementClassName error");
